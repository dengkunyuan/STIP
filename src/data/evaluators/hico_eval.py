# ------------------------------------------------------------------------
# HOTR official code : src/data/evaluators/hico_eval.py
...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat):
        ...
        for img_preds in preds:
            ...
            if len(subject_ids) > 0:
                ...
                hoi_scores *= masks ## remove impossible verb-obj pairs
    ...
    def evaluate(self):
        for img_id, (img_preds, img_gts) in enumerate(zip(self.preds, self.gts)):
            ...
            known_object_catids = [x['category_id'] for x in gt_bboxes]
            if len(gt_bboxes) != 0:
                bbox_pairs, bbox_overlaps = self.compute_iou_mat(gt_bboxes, pred_bboxes) ## match label and box
                self.compute_fptp(pred_hois, gt_hois, bbox_pairs, pred_bboxes, bbox_overlaps, known_object_catids=known_object_catids)
            else:
                if self.mode == 'default': # all predicted hois considered false positives
    ...
    def compute_fptp(self, pred_hois, gt_hois, match_pairs, pred_bboxes, bbox_overlaps, known_object_catids=None):
        ...
        if len(pred_hois) != 0:
            for pred_hoi in pred_hois:
                ...
                if triplet not in self.gt_triplets:
                    continue
                if self.mode == 'known_objects' and (triplet[1] not in known_object_catids):
                    continue
