...
class APRole(object):
    ...
    def add_data(self, h_box, o_box, score, i_box, i_act, p_box, p_act):
        ...
        if n_o == 0:
            ...
            for label in range(len(self.act_name)):
                ...
                if p_inds.sum() == 0: ## no such relation, all considered as FP
                    self.tp[label] = np.append(self.tp[label], np.array([0]*n_h))
                    self.fp[label] = np.append(self.fp[label], np.array([1]*n_h))
                    continue
                ...
                h_over_thresh = h_over_thresh & np.expand_dims(p_inds, axis=0) # (n_h, n_p)
                ...
                o_overlaps = compute_overlap(o_mapping_box, p_mapping_box) # --: matching object
                o_overlaps = np.diag(o_overlaps) # (n_h, )
                o_overlaps.setflags(write=1)
                if (~h_valid).sum() > 0:
                    o_overlaps[~h_valid] = 0 # (n_h, )

                no_role_inds = (p_box[assigned_p, 4] == -1) # (n_h, ) -- GT relation obj box is empty
                nan_box_inds = np.all(o_mapping_box == 0, axis=1) | np.all(np.isnan(o_mapping_box), axis=1) ## predicted relation obj box is 0 or nan
                ...
                if self.scenario_flag: ## scenario_1: for empty GT relation obj box, the predicted obj box should also be correct
                    o_iou_inds[no_role_inds & nan_box_inds] = 1
                    o_iou_inds[no_role_inds & ~nan_box_inds] = 0
                else: ## scenario_2: for empty GT relation obj box, ignore prediction for obj box
                    o_iou_inds[no_role_inds] = 1

                iou_inds = (h_iou_inds & o_iou_inds)
                p_nonzero = iou_inds.nonzero()[0]
                p_inds = assigned_p[p_nonzero]
                p_iou = np.unique(p_inds, return_index=True)[1] ## remove duplicate, one GT match only once
                p_tp = p_nonzero[p_iou]
    ...
    def evaluate(self, print_log=False):
        ...
        for label in range(len(self.act_name)):
            ...
            self.fp[label] = self.fp[label][indices] ## compute AP over all predicted relations across the dataset by ranking
