# ------------------------------------------------------------------------
# HOTR official code : src/models/post_process.py
        ...
        if self.HOIDet:
            if dataset == 'vcoco':
                ...
                for batch_idx, (s, l, b) in enumerate(zip(scores, labels, boxes)):
                    ...
                    o_inds = torch.cat((o_inds, torch.ones(1).type(torch.bool).to(o_inds.device))) # add an empty box
                    ...
                    # Score function: strange HOTR scoring
